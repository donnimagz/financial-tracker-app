...
Covers 1-month rent (1.7M), all 7 active subscriptions (318.2k), 1 month of debts (1.701M),
...
def generate_html(data):
    ...
    fixed_pct_a = (fixed_a / tot_a * 100) if tot_a else 0
    var_pct_a = (data["monthly_variable"] / tot_a * 100) if tot_a else 0
    ...
    debt_rows_html = "".join([f"""
        <tr>
            ...
            <td class="font-semibold text-zinc-100">{d['description']}</td>
...
<body class="p-6 max-w-5xl mx-auto text-sm">
    ...
    <header class="border-b border-zinc-800 pb-5 mb-6 flex justify-between items-start">
        <div>
            ...
            <p class="text-xs text-zinc-400 mt-0.5">30-Day Financial Blueprint • Rent (1.7M), Subscriptions (318.2k), 1 Month of Debts & Utilities</p>
        </div>
        <div class="text-right">
            <span class="inline-block px-2.5 py-1 bg-zinc-800 border border-zinc-700 text-zinc-300 rounded text-xs font-mono">1-MONTH EXECUTIVE REPORT</span>
    ...
    <div class="grid grid-cols-3 gap-4 mb-6">
        ...
        <div class="border border-rose-900/50 rounded-xl p-4 bg-rose-950/20">
            <div class="flex justify-between items-center mb-1">
                <span class="text-xs font-bold uppercase tracking-wider text-rose-400">Scenario A: 1-Mo Debt Payoff</span>
            ...
            <p class="text-xs text-zinc-400 mt-2">Clears 100% of all 6 active liabilities ({data['total_debt_owed']:,.0f} UGX) in 30 days.</p>
        ...
        <div class="border border-amber-900/50 rounded-xl p-4 bg-amber-950/20">
            <div class="flex justify-between items-center mb-1">
                <span class="text-xs font-bold uppercase tracking-wider text-amber-400">Scenario B: Amortized Debt</span>
            ...
            <p class="text-xs text-zinc-400 mt-2">Services debt at your actual Q3 average repayment pace ({data['debt_amortized']:,.0f} UGX/mo).</p>
        ...
        <div class="border border-emerald-900/50 rounded-xl p-4 bg-emerald-950/20">
            <div class="flex justify-between items-center mb-1">
                <span class="text-xs font-bold uppercase tracking-wider text-emerald-400">Scenario C: Debt-Free Baseline</span>
    ...
    <div class="page-break pt-4">
        ...
        <!-- 1-Month Debts Schedule -->
        <div class="bg-zinc-900/80 border border-zinc-800 rounded-xl p-5 mb-6">
            <div class="flex justify-between items-center mb-3">
                <div>
                    <h3 class="font-bold text-xs uppercase tracking-wider text-rose-400">💳 Active 1-Month Debt Obligations (1,701,360 UGX Total)</h3>
                    <p class="text-[11px] text-zinc-400">Complete itemization of all 6 active liabilities scheduled for 1-month liquidation.</p>
                </div>
            ...
            <table class="w-full text-xs">
                <thead>
                    <tr class="border-b border-zinc-800 text-zinc-500 text-left">
                        <th class="py-2 font-medium">Date Booked</th>
                        <th class="py-2 font-medium">Creditor / Description</th>
                        <th class="py-2 font-medium text-center">Status</th>
                        <th class="py-2 font-medium">Account</th>
                        <th class="py-2 font-medium text-right">Amount (UGX)</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-zinc-800/60">
                    {debt_rows_html}
        ...
        <div class="grid grid-cols-2 gap-4 mb-6">
            <div class="bg-zinc-900/80 border border-zinc-800 rounded-xl p-4">
                ...
                <p class="text-xs text-zinc-400 mb-2">Non-negotiable monthly residential commitment.</p>
                <div class="p-2.5 rounded bg-zinc-950/50 border border-zinc-800/80 text-xs flex justify-between items-center">
            ...
            <div class="bg-zinc-900/80 border border-zinc-800 rounded-xl p-4">
                ...
                <p class="text-xs text-zinc-400 mb-2">Estimated 30-day baseline based on Q3 consumption.</p>
                <div class="space-y-1.5 text-xs">
                    <div class="p-2 rounded bg-zinc-950/50 border border-zinc-800/80 flex justify-between items-center">
            ...
            <table class="w-full text-xs">
                <thead>
                    <tr class="border-b border-zinc-800 text-zinc-500 text-left">
                        ...
                        <th class="py-2 font-medium">Subcategory</th>
                        <th class="py-2 font-medium text-right">Estimated 1-Mo Spend</th>
                        <th class="py-2 font-medium text-right">% of Total Month</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-zinc-800/60">
                    {var_rows_html}
        ...
        <div class="bg-emerald-950/20 border border-emerald-800/40 rounded-xl p-4 text-xs text-zinc-300">
            ...
            <ul class="list-disc list-inside space-y-1 text-zinc-400">
                <li><strong class="text-zinc-200">Liquidation Sprint (Scenario A):</strong> Committing <strong>6,367,103 UGX</strong> over the next 30 days permanently eliminates all 6 credit balances (Vernon 750k, MoKash 327k, Zenka 203.4k, Health Okay 163k, Maureen Asio 163k, Benon 95k).</li>
                <li><strong class="text-zinc-200">Post-Debt Living Baseline (Scenario C):</strong> Once liabilities are paid off, your ongoing monthly burn rate contracts to <strong>4,665,743 UGX/month</strong>, creating an immediate monthly liquidity surplus of 1.70M UGX.</li>
                <li><strong class="text-zinc-200">Lean Subscription Stack:</strong> Pruning DaVinci, Netflix, and Spotify and standardizing Adobe ($25) and personal iCloud ($25) keeps essential digital infrastructure capped at <strong>318,200 UGX/month</strong> (5.0% of total budget).</li>
            </ul>
        </div>
    </div>
...
def main():
    ...
    cmd = [
        ...
        "--user-data-dir=/tmp/chrome_pdf_profile_monthly",
